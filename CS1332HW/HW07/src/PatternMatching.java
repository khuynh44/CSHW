...
public class PatternMatching {
    ...
    public static List<Integer> kmp(CharSequence pattern, CharSequence text,
        ...
        if (pattern == null || pattern.length() == 0) {
            throw new IllegalArgumentException("Pattern"
                    + " cannot be null or have length of 0");
        }
        ...
        int k = 0;
        while (k < text.length()
                && text.length() - k - 1 >= pattern.length() - j - 1) {
    ...
    public static List<Integer> boyerMoore(CharSequence pattern,
        ...
        if (pattern == null || pattern.length() == 0) {
            throw new IllegalArgumentException("Pattern"
                    + " cannot be null or have length 0");
        }
        if (text == null || comparator == null) {
            throw new IllegalArgumentException("Text"
                    + " or comparator cannot be null");
    ...
    public static List<Integer> rabinKarp(CharSequence pattern,
        ...
        for (int i = 0; i < pattern.length(); i++) {
            pHash = pHash + pattern.charAt(i) * (pow1);
            tHash = tHash + text.charAt(i) * (pow1);
            pow1 = pow1 / BASE;
        }
        for (int i = 0; i < text.length() - pattern.length() + 1; i++) {
            ...
            tHash = (tHash - (text.charAt(i) * (pow))) * BASE
