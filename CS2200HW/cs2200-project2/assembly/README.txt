The LC-901a Assembler and Simulator
===============

To aid in testing your processor, we have provided an *assembler* and
a *simulator* for the LC-901a architecture. The assembler supports
...
the assembler. The LC-901a assembler definition is included.

Included Files
-----------

* `assembler.py`: the main assembler program
* `lc901a.py`: the LC-901a assembler definition
* `lc901a-sim.py`: the LC-901a simulator program
...
Example usage to assemble `test.s`:
    ...
    python assembler.py -i lc901a --sym test.s
...
To output assembled code in hexadecimal (for use with *CircuitSim*):
    ...
    python assembler.py -i lc901a --hex test.s
...
In addition to the syntax described in the LC-901a ISA reference,
